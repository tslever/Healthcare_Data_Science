...
def main(name_of_model, number_of_training_or_testing_observations, path_to_data_frame):
    ...
    series_of_observed_response_values = data_frame_of_observed_response_values_and_average_of_predicted_response_values['actual_los']
    for pair in dictionary_of_numbers_of_standard_deviations_below_mean_and_colors.items():
        df = data_frame_of_observed_response_values_and_average_of_predicted_response_values[['actual_los']].copy()
        z_score = pair[0]
        ...
        df = df.sort_values(by = 'actual_los')
        df.reset_index(drop = True, inplace = True)
        df.drop('actual_los', axis = 1, inplace = True)
        ...
        print(df.head(n = 3))
        plt.plot(df.index, df['cumulative_relative_frequency_of_indicators_that_observed_response_value_is_below_threshold_for_perfect_model'], label = f'perfect model; z score = {z_score}', color = 'blue')
        df = data_frame_of_observed_response_values_and_average_of_predicted_response_values.copy()
        df['indicator_that_observed_response_value_is_below_threshold'] = [1 if observed_response_value < threshold else 0 for observed_response_value in series_of_observed_response_values]
        df.drop('actual_los', axis = 1, inplace = True)
        df.sort_values(by = 'predicted_expected_los', inplace = True)
        df.reset_index(drop = True, inplace = True)
        df.drop('predicted_expected_los', axis = 1, inplace = True)
        ...
        print(df.head(n = 3))
        plt.plot(df.index, df['cumulative_relative_frequency_of_positive_indicators_that_observed_response_value_is_below_threshold_for_trained_model'], label = f'trained model; z score = {z_score}', color = 'green')
    plt.grid()
    plt.legend()
